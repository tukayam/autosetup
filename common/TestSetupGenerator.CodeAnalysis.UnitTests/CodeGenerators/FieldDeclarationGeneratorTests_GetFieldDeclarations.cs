...
namespace TestSetupGenerator.CodeAnalysis.UnitTests.CodeGenerators
{
    public class FieldDeclarationGeneratorTests_GetFieldDeclarations
    {
        ...
        {
            var typeSyntax = SyntaxNodeProvider.GetSyntaxNodeFromFile<ParameterSyntax>(filePath, typeName);
            var syntaxGenerator = new SyntaxGeneratorProvider().GetSyntaxGenerator();

            var actual = _target.GetFieldDeclarations(new List<ParameterSyntax> { typeSyntax }, syntaxGenerator).ToList();
        ...
        public void Returns_FieldDeclarations_ForEach_ConstructorParameterInClassUnderTest()
        {
            ...
            var classDeclarationSyntax = SyntaxNodeProvider.GetSyntaxNodeFromFile<ClassDeclarationSyntax>(filePath, className);
            var syntaxGenerator = new SyntaxGeneratorProvider().GetSyntaxGenerator();
